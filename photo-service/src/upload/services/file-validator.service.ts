...
import { AllowedMimeType, HEIC_BRANDS, HEIF_BRANDS, MIME_TYPES } from '../constants';
...
export class FileValidatorService {
  ...
  private detectTiffVariant(buffer: Buffer): AllowedMimeType {
    ...
    const header = buffer.subarray(0, Math.min(1024, buffer.length)).toString('ascii');
  ...
  private detectHeicHeif(buffer: Buffer): AllowedMimeType | null {
    ...
    if (buffer.length > 16) {
      const compatBrands = buffer.subarray(16, Math.min(64, buffer.length)).toString('ascii');
      if (HEIC_BRANDS.some((b) => compatBrands.includes(b))) return MIME_TYPES.HEIC;
      if (HEIF_BRANDS.some((b) => compatBrands.includes(b))) return MIME_TYPES.HEIF;
    }
