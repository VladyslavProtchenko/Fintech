...
export class BalanceService {
  ...
  async topup(userId: string, amount: string): Promise<MappedTransaction> {
    const user = await this.ensurePaymentClient(userId);
    try {
  ...
  async transfer(
    ...
    if (sender.id === recipientUser.id) throw new BadRequestException('Cannot transfer to yourself');
    ...
    try {
      ...
      return this.mapTransaction(
        tx,
        sender.walletId!,
        new Map([[recipient.walletId!, recipient.name]]),
      );
    } catch (err) {
