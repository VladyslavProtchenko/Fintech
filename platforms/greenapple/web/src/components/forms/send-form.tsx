...
import { sendAction } from '@/actions/payment';
...
export function SendForm() {
  ...
  const [sendError, sendFormAction, pending] = useActionState(sendAction, null);
  ...
  function handleSearch() {
    ...
    startSearch(async () => {
      ...
      if (!result || !result.found) {
        setSearchError('No GreenApple member found with that email');
        return;
      }
  ...
  if (step === 'done' && !sendError) {
    return (
      <div className="text-center py-8 space-y-4">
        ...
        <p className="text-gray-500 text-sm">Your funds are on their way to {recipientName}</p>
        <Link href="/dashboard" className="block text-sm text-green-600 font-semibold hover:underline">
          Back to Dashboard
        </Link>
      </div>
    );
      ...
      <div className={step !== 'email' ? 'opacity-50 pointer-events-none' : ''}>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-400 text-sm"
          ...
          <button
            ...
            className="px-4 py-2.5 text-sm font-bold text-white bg-green-500 rounded-xl hover:bg-green-600 disabled:opacity-50 transition-colors"
          >
            {isSearching ? '…' : 'Find'}
          </button>
        </div>
      ...
      {step === 'amount' && (
        ...
        >
          ...
          <div className="bg-green-50 rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-green-800">Sending to: {recipientName}</span>
            <button
              type="button"
          ...
          >
            {pending ? 'Sending…' : '↗ Send to Friend'}
          </button>
        </form>
      )}
